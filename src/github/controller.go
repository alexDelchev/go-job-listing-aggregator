...
func newContoller(scheduler *Scheduler, router *mux.Router) controller {
	...
	router.HandleFunc("/modules/github/scheduler/start", newController.startScheduler).Methods("POST")
	router.HandleFunc("/modules/github/scheduler/stop", newController.stopScheduler).Methods("DELETE")
